...
import {
  ...
  listEngineeringStages,
  listEngineeringApprovals,
  uploadEngineeringDrawing,
  advanceEngineeringStage,
  decideEngineeringApproval,
  listBomItems,
...
export const appRouter = router({
  ...
  production: router({
    ...
    createEngineeringItem: protectedProcedure.input(z.object({ itemCode: z.string().min(2).max(48), title: z.string().min(3).max(180), revision: z.string().min(1).max(12), ownerName: z.string().min(2).max(120) })).mutation(({ ctx, input }) => createEngineeringItem(ctx.user.id, input)),
    stages: protectedProcedure.input(z.object({ itemId: z.number().int().positive() })).query(({ ctx, input }) => listEngineeringStages(ctx.user.id, input.itemId)),
    approvals: protectedProcedure.input(z.object({ itemId: z.number().int().positive() })).query(({ ctx, input }) => listEngineeringApprovals(ctx.user.id, input.itemId)),
    uploadDrawing: protectedProcedure.input(z.object({ itemId: z.number().int().positive(), fileName: z.string().min(1).max(180).refine((name) => /\.(pdf|dxf)$/i.test(name), "Only PDF and DXF files are supported"), contentType: z.enum(["application/pdf", "application/dxf", "image/vnd.dxf", "application/octet-stream"]), base64: z.string().min(10).max(20_000_000) })).mutation(({ ctx, input }) => uploadEngineeringDrawing(ctx.user.id, input.itemId, input.fileName, input.contentType, input.base64)),
    completeStage: protectedProcedure.input(z.object({ itemId: z.number().int().positive(), stageId: z.number().int().positive(), notes: z.string().max(1000).optional() })).mutation(({ ctx, input }) => advanceEngineeringStage(ctx.user.id, input.itemId, input.stageId, ctx.user.name ?? "Engenharia", input.notes)),
    decideApproval: protectedProcedure.input(z.object({ itemId: z.number().int().positive(), approvalId: z.number().int().positive(), status: z.enum(["Aprovado", "Rejeitado"]), comment: z.string().max(1000).optional() })).mutation(({ ctx, input }) => decideEngineeringApproval(ctx.user.id, input.itemId, input.approvalId, input.status, ctx.user.name ?? "Aprovador", input.comment)),
